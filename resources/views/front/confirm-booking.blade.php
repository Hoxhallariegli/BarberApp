        ...
        <div class="bg-[#1E1E1E] p-8 rounded-[2.5rem] shadow-2xl border border-white/5 text-center relative overflow-hidden">
            ...
            @if(session('success') || $booking->status === 'confirmed')
                <div class="py-6 space-y-4">
                    ...
                    <h2 class="text-2xl font-bold italic uppercase tracking-tight">U konfirmua!</h2>
                    <p class="text-gray-400 text-sm leading-relaxed">
                        Faleminderit {{ $booking->customer_name }}.<br>
                        Takimi juaj për në orën <span class="text-white font-bold">{{ $booking->appointment_datetime->format('H:i') }}</span> është gati.
                    </p>
                    <div class="pt-6">
                        <a href="/" class="text-brass text-xs font-black uppercase tracking-[0.2em] hover:text-white transition">Kthehu te faqja</a>
                    </div>
                </div>
            ...
            @elseif(session('info') || $booking->status === 'cancelled')
                <div class="py-6 space-y-4">
                    ...
                    <h2 class="text-2xl font-bold italic uppercase tracking-tight text-white/60">U anullua</h2>
                    <p class="text-gray-500 text-sm leading-relaxed italic">
                        Rezervimi u fshi me sukses.<br>Mirupafshim herën tjetër!
                    </p>
                    <div class="pt-6">
                        <a href="/" class="text-brass text-xs font-black uppercase tracking-[0.2em] hover:text-white transition">Kthehu te faqja</a>
                    </div>
            ...
            @else
                ...
                <div class="space-y-6">
                    ...
                    <p class="text-gray-400 text-sm leading-relaxed">
                        Keni rezervuar sot në orën <span class="text-white font-bold">{{ $booking->appointment_datetime->format('H:i') }}</span>.<br>
                        A do të paraqiteni në takim?
                    </p>

                    <div class="flex flex-col gap-3 pt-4">
                        <form action="{{ url('/confirm/'.$booking->token.'/yes') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-brass hover:bg-brass/90 text-black font-black uppercase tracking-widest py-4 rounded-2xl transition shadow-lg shadow-brass/20 active:scale-95">
                                Po, do të vij ✅
                            </button>
                        </form>
                        ...
                        <form action="{{ url('/confirm/'.$booking->token.'/no') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-widest py-4 rounded-2xl transition active:scale-95">
                                Jo, nuk mundem ❌
                            </button>
                        </form>
                    </div>
