    ...
    <div class="card !p-0 overflow-hidden shadow-none border-gray-200 dark:border-gray-700 dark:bg-gray-800">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div><x-h1>{{ __('payments.Payments') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('payments.List of') }} {{ strtolower(__('payments.Payments')) }}</x-short-description></div>
            ...
            <div x-show="openFilter" x-cloak class="mt-6 p-6 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-2xl">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input name="search" wire:model.live.debounce.300ms="search" type="text" :placeholder="__('payments.Search') . '...'" class="w-full p-3 text-sm font-bold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl focus:ring-2 focus:ring-blue-500/20 dark:text-white">
                    </div>
                    <div><label class="block mb-1.5 text-[10px] font-bold uppercase tracking-widest ml-1 text-gray-900 dark:text-gray-100">{{ __('payments.Booking Id') }}</label><x-form.dropdown-search name="booking_id" wire:model.live="booking_id" label="none" :data="$bookings" :placeholder="__('payments.Filters') . ' ' . __('payments.Booking Id')" /></div>
                </div>
            </div>
        </div>
