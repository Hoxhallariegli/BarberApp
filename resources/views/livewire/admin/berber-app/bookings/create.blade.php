<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('bookings.Add Booking') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('bookings.New record') }}</x-short-description></div><x-back-btn route="admin.bookings.index" /></div>
...
<div><x-form.input name="appointment_datetime" type="datetime-local" wire:model="appointment_datetime" :label="__('bookings.Appointment Datetime')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('bookings.Save') }}</x-button></div></form></div>
</div>
