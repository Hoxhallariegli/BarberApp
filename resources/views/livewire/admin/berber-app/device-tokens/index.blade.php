    ...
    <div class="card !p-0 overflow-hidden shadow-none border-gray-200 dark:border-gray-700 dark:bg-gray-800">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @endif
                    <button @click="openFilter = !openFilter" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-gray-600 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm transition-none"><span>{{ __('device-tokens.Filters') }}</span></button>
                    <x-btn :href="route('admin.device-tokens.create')" icon="plus">{{ __('device-tokens.Add DeviceToken') }}</x-btn>
                </div>
            </div>
        ...
        <div class="overflow-x-auto border-t border-gray-100 dark:border-gray-700">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="bg-gray-100/50 dark:bg-gray-700/50"><tr><x-table.th name="id" :label="__('device-tokens.ID')" :$sortField :$sortAsc :sortable="true" /><x-table.th name="user_id" :label="__('device-tokens.User Id')" :$sortField :$sortAsc :sortable="in_array('user_id', $sortableFields)" />
<x-table.th name="fcm_token" :label="__('device-tokens.Fcm Token')" :$sortField :$sortAsc :sortable="in_array('fcm_token', $sortableFields)" />
<x-table.th name="device_type" :label="__('device-tokens.Device Type')" :$sortField :$sortAsc :sortable="in_array('device_type', $sortableFields)" /><th class="px-6 py-4 text-right text-[10px] font-black uppercase text-gray-400 tracking-widest">{{ __('device-tokens.Action') }}</th></tr></thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">@forelse($items as $item) <livewire:admin.berber-app.device-tokens.row :$item :key="$item->id" /> @empty <tr><td colspan="100" class="px-6 py-10 text-center text-sm text-gray-400">{{ __('device-tokens.No records found.') }}</td></tr> @endforelse</tbody>
            </table>
        </div>
