        ...
        foreach ($fields as $field) {
            if (Str::endsWith($field, '_id')) {
                ...
                if (method_exists($modelClass, $method)) {
                    $relations[$field] = ['method' => $method, 'endpoint' => Str::plural(Str::kebab($base)), 'label' => Str::headline($method)];
                }
            }
        }
        return $relations;
...
DART;
    ...
    if({$f}Data is Map) { _{$f}LangControllers['sq']!.text = {$f}Data['sq'] ?? ''; _{$f}LangControllers['en']!.text = {$f}Data['en'] ?? ''; }\n";
        ...
        $saveCall = $hasImage
            ...
            : "widget.item == null ? await ApiService.post('/##PLURAL_KEBAB##', payload) : await ApiService.put('/##PLURAL_KEBAB##/\${widget.item!['id']}', payload)";
